...
from src.agents.nn_agent import NeuralNetwork, NeuralNetworkAgent
...
def run_aivai_game(deck_size: int = 5, hand_size: int = 2):
    ...
    agents: Dict[str, BaseAgent] = {
        player1_id: RandomAgent(player1_id), 
        player2_id: NeuralNetworkAgent(player2_id, neural_network)
    }
...
if __name__ == "__main__":
    ...
    print(f"AI1 wins: {player1_wins} ({player1_wins/num_games:.1%})")
    print(f"AI2 wins: {player2_wins} ({player2_wins/num_games:.1%})")
    ...
    # import math
    # digits = math.ceil(math.log10(num_games))

    # for index, logs in enumerate(results_log):
    #     # Create a timestamp for the filename
    #     timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    #     filename = f"game_log_{index:0{digits}d}.json"
